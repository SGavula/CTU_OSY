...
bool is_work_possible(int workplace_id) {
    ...
    for(int i = start; i < end; i++) {
        ...
        if(y_temp > 0) {
            ...
            // // printf("X: Y: %d %d\n", x_temp, y_temp);
            // // printf("Value: %d\n", products_table[x_temp][y_temp]);
            // Return false if worker in previous stage is not present or if workplace is not present
...
void is_products_makeable() {
    for(int i = 0; i < _PRODUCT_COUNT; i++) {
        for(int j = 0; j < _PHASE_COUNT; j++) {
            int workplace_id = products_table_info[i][j];

            if(workplace.num_of_workers[workplace_id] == 0 || workplace.num_of_workplaces[workplace_id] == 0) {
...
void* worker_function(void *arg) {
    ...
    while (1) {
        ...
        // If input ended and worker has no work, then end
        if(end_of_app == true && is_work_possible(worker.workplace) == false) {
        ...
        while (workplace.num_of_workplaces[worker.workplace] == 0 || check_for_item(worker.workplace, &x, &y) == 1) {
            ...
            if(end_of_app == true) {
                if(workplace.queue[worker.workplace] == 0 || is_work_possible(worker.workplace) == false) {
                    ...
                    // for(int i = 0; i < 3; i++) {
                    //     int value = possible_finish[i] == true ? 0 : 1;
                    //     printf("%i ", value);
                    // }
                    // printf("\n");
                    // printf("X: %d\n", x);
                    // Print if product cannot be finished
                    if(possible_finish[x] == false) {
                        printf("%s goes home\n", worker.name);
                    }

                    // printf("worker.workplace: %d\n", worker.workplace);
                    // printf("workplace.queue[worker.workplace]: %d\n", workplace.queue[worker.workplace]);
                    // printf("X: %d\n", x);
                    // Worker on the lakovna can leave after done product A or C
                    if(worker.workplace == 4 && workplace.queue[worker.workplace] == 0 && x != 1) {
                        print_workers_on_completion();
                    }
                    // Worker on the sroubovak can leave after done product B
                    if(workers[worker_id].workplace == 5 && workplace.queue[workers[worker_id].workplace] == 0 && x == 1) {
                        print_workers_on_completion();
                    }
                    pthread_mutex_unlock(&mutex_print);
                    pthread_mutex_unlock(&mutex_work);
                    printf("Worker end with name: %s\n", worker.name);
                    return NULL;
                }
            }
            // UNLOCK
        ...
        if((y+1) < _PHASE_COUNT) {
            ...
            // printf("WORKER THAT PRINT THIS LINE: %s\n", worker.name);
            printf("done %s\n", product_str[x]);
            print_workers_on_completion();
        }
        // print_products_table();
