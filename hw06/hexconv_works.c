...
static void print(unsigned num)
{
    ...
    // int ret = custom_write(STDOUT_FILENO, buf, custom_strlen(buf));
    int ret = custom_write(buf, custom_strlen(buf));
    if (ret == -1) {
        custom_print("Custom write\n");
        custom_exit(1);
    }
}
...
int _start()
{
    ...
    for (; ; i++, chars_to_process--) {
        if (chars_to_process == 0) {
            // int ret = custom_read(STDIN_FILENO, buf, sizeof(buf));
            int ret = custom_read( buf, sizeof(buf));
            if (ret < 0) {
                custom_print("Custom read\n");
                custom_exit(1);
            }
            i = 0;
